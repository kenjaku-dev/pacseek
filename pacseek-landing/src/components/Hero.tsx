...
export function Hero() {
  ...
  return (
    <section id="top" className="relative overflow-hidden">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_70%_20%,rgba(34,197,94,0.07),transparent)]"
      />

      <div className="relative mx-auto grid w-full max-w-5xl items-center gap-10 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-[1fr_1.05fr] lg:gap-12 lg:py-24">
        <div className="max-w-xl">
          <motion.p
            initial={reduce ? false : { opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.35, ease }}
            className="mb-4 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-[11px] text-muted-foreground"
          >
          ...
          <motion.h1
            ...
            className="text-3xl font-semibold leading-[1.15] tracking-tight text-foreground sm:text-4xl lg:text-[2.75rem]"
          >
            Search Arch packages
            <span className="block text-accent">at terminal speed.</span>
          </motion.h1>
          ...
          >
            One binary for <span className="text-foreground">pacman -Ss</span> and AUR
            RPC v5. Search in parallel, preview the PKGBUILD, install or remove —
          ...
          >
            <a
              href="#install"
              className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-accent px-4 py-2.5 text-sm font-semibold text-on-accent transition-opacity duration-200 hover:opacity-90"
            >
              Get pacseek
          ...
          >
            <code className="rounded-md border border-border bg-card px-2.5 py-1.5 text-[11px] text-accent-secondary sm:text-xs">
              {INSTALL_CMD}
            </code>
